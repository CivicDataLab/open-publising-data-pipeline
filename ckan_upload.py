        ...
        for state_dir in glob.glob("%s/*" % input_dir):
            ...
            for year_dir in glob.glob("%s/*" % state_dir):
                ...
                for dataset_dir in dataset_list:
                    ...
                    dataset_name = re.sub(
                        r"^\d{,2}\s{0,}[\.\-]{,1}\s{0,}", "", dataset_name
                    )
                    if re.findall(r" - ", dataset_dir):
                        dataset_name = dataset_name.split(" - ")[0]
                    dataset_handle = self.org_creator.get_org_handle(dataset_name)
                    dataset_handle = self.get_modified_handle(
                        dataset_handle, state_slug, year_name
                    )
                    print("a", dataset_handle)
                    dataset_name = self.get_modified_dataset(
                        dataset_name, state_name, year_name
                    )
                    ...
                    try:
                        response = requests.post(
                            package_url, headers=auth_dict, data=json.dumps(payload)
                        )
                        ...
                        print(
                            "Unable to upload dataset: %s, error_message: %s"
                            % (dataset_handle, e)
                        )
                    ...
                    for file_type in upload_list:
                        try:
                            ...
                            print(
                                "Unable to upload resource: %s, error_message: %s"
                                % (dataset_name, e)
                            )
