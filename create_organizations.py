    ...
    def create_organizations_from_file(self, base_url, auth_key, input_file_path):
        ...
        for title in titles:
            self.create_organization(
                base_url, auth_key, title, "expenditure-union-budget"
            )
    ...
    def create_organization(
        ...
        try:
            response = requests.post(
                resource_url, headers=auth_dict, data=payload_json, verify=False
            )
            ...
            print(
                "Unable to upload for department: %s, error_message: %s"
                % (title, error_message)
            )
        # print(command)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_file_path", help="Input text filepath containing name of organizations"
    )
    args = parser.parse_args()
    obj = CreateOrganizations()
    if not args.input_file_path or not args.base_url or not args.auth_key:
        print(
            "Please pass required arguments: base_url, auth_key and input_pdf_filepath"
        )
    else:
        obj.create_organizations_from_file(
            args.base_url, args.auth_key, args.input_file_path
        )
