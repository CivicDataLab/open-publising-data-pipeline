...
from projects.dpg_pipeline.mgnrega_flow.data_for_districts_flow import \
    data_for_districts_pipeline
...
def task_executor(pipeline_id, data_url, project):
    ...
    try:
        ...
        print(
            "received tasks from POST request..for..", new_pipeline.model.pipeline_name
        )
        ...
        if project == "generic_transformations":
            prefect_tasks.pipeline_executor(
                new_pipeline
            )  # pipeline_executor(task.task_name, context)
        elif project == "d4d":
